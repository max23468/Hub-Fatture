...
All'avvio lo stack applica le migrazioni. La prima configurazione usa `/setup`, crea atomicamente gli account fissi `Massimo` e `Codex` e richiede password di almeno 8 caratteri. Il login non distingue maiuscole e minuscole, mentre interfaccia e audit mostrano sempre i nomi canonici. Il token di bootstrap Development è generato una volta e conservato nel Portachiavi macOS; la password Development di `Codex` resta in una voce separata. Nessuno dei due valori entra nel repository. In ambienti condivisi i valori di `.env.example` devono provenire dal secret store.
