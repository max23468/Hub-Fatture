...
L'ambiente resta disponibile su `http://localhost:8080` anche fra sessioni di lavoro e riparte con Colima dopo il riavvio del Mac. Le modifiche al codice vengono caricate automaticamente; PostgreSQL e `node_modules` usano volumi persistenti. Il comando conserva nel Portachiavi macOS la chiave di cifratura e il token di bootstrap Development; riusa la chiave sia in Compose sia con Shopify CLI. Il database persistente è raggiungibile dal solo Mac su `127.0.0.1:5432`. Il servizio `app-worker` gestisce webhook e sincronizzazioni periodiche. Usa `docker compose logs -f app app-worker` per seguire i processi e `docker compose stop` soltanto quando vuoi fermare esplicitamente l'ambiente. Non usare `docker compose down -v`, perché elimina i dati locali.

All'avvio lo stack applica le migrazioni. La prima configurazione usa `/setup`, crea atomicamente gli account fissi `Massimo` e `Codex` e richiede password di almeno 8 caratteri. Il login non distingue maiuscole e minuscole, mentre interfaccia e audit mostrano sempre i nomi canonici. Il token di bootstrap Development è generato una volta e conservato nel Portachiavi macOS; le password dei due account restano in voci separate per account e ambiente. Nessun valore entra nel repository. I nomi esatti delle voci Production e la procedura di rotazione sono documentati nell'[inventario segreti](docs/runbooks/secret-inventory.md). In ambienti condivisi i valori di `.env.example` devono provenire dal secret store.
