...
test("la policy Pubblica resta coerente nelle fonti canoniche", async () => {
  ...
  assert.match(agents, /serie stabile `1\.x`/);
  assert.doesNotMatch(agents, /versioni applicative fino alla `1\.0\.0`/);
  assert.match(agents, /Definition of Done[\s\S]*baseline completata/);
  ...
  assert.match(agents, /passaggio da disabilitata ad abilitata/);
...
test("gli script Production sono sintatticamente validi e conservano i gate di continuità", async () => {
  ...
  assert.match(readinessQuery, /submissions\.environment = 'PRODUCTION'/);
  assert.match(readinessQuery, /submissions\.status = 'UNKNOWN_REMOTE_STATE'/);
  assert.match(readinessQuery, /batches\.requires_reconciliation/);
  assert.match(readinessQuery, /'DOCUMENT_ONLY'/);
  ...
  assert.match(readinessQuery, /status NOT IN \('RECONCILED', 'CANCELLED', 'DOCUMENT_ONLY'\)/);
