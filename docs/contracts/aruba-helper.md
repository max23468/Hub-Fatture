...
- L’unica eccezione è l’invio pilota esplicitamente autorizzato dal titolare: parte da un batch assistito ancora intatto di un solo documento, crea un nuovo batch `AUTOMATIC` con un manifest distinto e un solo permesso interno `CANARY`, quindi annulla il batch sorgente senza modificarne il manifest. Il flag globale deve restare `false`.
- Può esistere un solo permesso pilota non consumato e non revocato. Il server serializza preparazione e rinnovo, revoca i permessi scaduti, impedisce la preparazione concorrente e rifiuta il consumo se ambiente, modalità, batch, manifest, documento, revisione, hash, validazione, scadenza o stato del kill switch non coincidono.
