...
Il manifest immutabile di upload comprende ambiente, modalità, riferimento non segreto dell’account atteso, tentativo e, per ogni documento, ID, revisione, hash XML, nome file, dimensione, numero fiscale, data e totale. Il manifest separato della scansione in sola lettura comprende inoltre l’identità visibile attesa, distinta dal riferimento usato per partizionare i dati, e deve coincidere con l’unica descrizione mostrata dal controllo account nella barra superiore Aruba. Gli XML scaricati dall’helper vengono accettati soltanto se dimensione e SHA-256 coincidono.
...
Il pannello sintetico espone lo scenario `?scenario=inventory`. Il preferito di sincronizzazione dell'inventario è coperto su Chromium e WebKit; Safari reale deve comunque essere qualificato sul pannello Aruba prima del go-live. L'helper distinto di caricamento e trasmissione continua a richiedere Chrome o Edge con un profilo Chromium persistente multipiattaforma.
