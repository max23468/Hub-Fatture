...
| Token e permessi Aruba          | hash dei codici helper e permessi monouso scaduti, revocati o consumati                                              | 30 giorni dall’evento conclusivo; un permesso pilota resta finché il batch non è terminale      | cancellazione; l’audit critico collegato resta secondo la propria classe                             |
...
| Backup OCI                      | archivi giornalieri cifrati immutabili                                                                               | 35 giorni; l’oggetto protetto `current` è escluso dal lifecycle                                | lifecycle OCI sugli archivi, senza possibilità di eliminare l’ultimo backup valido                   |
...
1. Un job giornaliero calcola i candidati usando l’orologio PostgreSQL e produce conteggi sanitizzati per classe senza contenuti personali.
...
L’implementazione applicativa aggiunge migrazione, job giornaliero transazionale, hold espliciti, audit sanitizzato, test PostgreSQL e retention degli access log. Prima della transazione, il job Production rilegge fail-closed la ricevuta montata del backup verificato e rifiuta ricevute assenti, non valide, future o più vecchie di 36 ore. La prova usa soltanto dati sintetici e verifica almeno permanenza dell’audit fiscale, hold attivo, redazione e-mail, payload provider, job conclusi e credenziali Aruba scadute. Lifecycle OCI, integrità remota del backup e copia Mac restano verifiche provider separate: non sono simulati dal job applicativo e devono essere riletti sul candidato distribuito.
