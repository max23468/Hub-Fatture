...
Il processo non possiede credenziali Object Storage statiche. L’Instance Principal può creare e rileggere soltanto gli oggetti cifrati del bucket per verificarne dimensione e metadati, ma non può cancellarli. Ogni esecuzione crea un giornale immutabile cifrato con dump PostgreSQL, manifest e ricevuta di deploy, quindi aggiorna la copia cifrata protetta e autonoma `current`, che contiene anche tutti i documenti. Dimensione e checksum dei due oggetti vengono verificati separatamente e registrati nella ricevuta. Il lifecycle elimina soltanto i giornali scaduti e non può selezionare `current`, quindi non può eliminare l’ultimo backup completo valido. Questa separazione evita di duplicare ogni giorno l’intero archivio documentale senza indebolire l’RPO del restore corrente.

Il monitor locale somma le dimensioni degli oggetti tramite lo stesso Instance Principal e avvisa al raggiungimento di 15 GB, margine prudenziale rispetto ai 20 GB inclusi verificati per Object Storage. Il dimensionamento considera 35 giornali database immutabili e una sola copia completa `current`; un nuovo giornale deve quindi restare materialmente più piccolo della copia completa. Un errore di lettura della quota è a sua volta un’anomalia; `OCI_BACKUP_WARNING_BYTES` serve soltanto per il collaudo controllato della soglia. Il timer resta fail-closed e conserva l’allarme finché la capacità non rientra, mentre il readback Production usa la modalità di sola segnalazione: un allarme prudenziale emerso dopo health check, ricevuta e digest verificati non retrocede un deploy già riuscito.
