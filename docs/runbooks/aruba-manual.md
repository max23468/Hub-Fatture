...
## Documenti creati direttamente nel pannello

Le API non aggiornano il contatore dei sezionali: dopo un documento emesso da Hub Fatture il pannello continua a proporre il progressivo successivo all’ultimo documento creato nel pannello stesso. Prima di creare manualmente una fattura, leggere in **Fatture inviate** l’ultimo numero `FPR` dell’anno, comprese note di credito e documenti scartati. Nella creazione guidata modificare il progressivo con **Ok, gestisci numerazione**, inserire il numero successivo e usare una data non anteriore all’ultimo documento. Inviare senza salvare in bozze, perché alla riapertura della bozza Aruba ripristina il progressivo proposto. Alla sincronizzazione successiva Hub Fatture acquisisce il documento e numera i propri dopo di esso. Fonte: [sezionale e progressivo](https://guide.aruba.it/soluzioni-fatturazione-elettronica/fe/fatture-documenti/dati-documento-clienti-prodotti-profilo-fiscale/campi-fattura-sezionale-progressivo).
