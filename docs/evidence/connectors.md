...
- Il keyset di destinazione è `botCF 2`; `EBAY_ACCOUNT_REFERENCE` identifica invece l'account venditore che la Identity API deve confermare e conserva il valore Production già verificato durante la rotazione. SyncBay mantiene l’unico RuName attivo e trasferisce il refresh token al comando interno di Hub Fatture tramite standard input. SyncBay riceve inoltre le notifiche Marketplace Account Deletion del keyset e dispone del relay durevole verso l'endpoint HTTPS pubblico di Hub Fatture. Hub Fatture verifica nuovamente la firma e cancella soltanto i dati del compratore non fiscalizzati.
- Contract check: `npm test -- src/integrations/connectors.test.ts`.

Le fixture versionate sono sintetiche e anonimizzate. Il readback storico tramite il precedente keyset `botCF` ha confermato `buyer.taxIdentifier` con tipo `CODICE_FISCALE` e due forme di `paymentSummary.refunds`: entrambe espongono `refundReferenceId` e `amount.value`/`amount.currency`, mentre `refundId` è opzionale. Il passaggio a `botCF 2` richiede una nuova prova della stessa forma prima di chiudere il cambio. Nessun identificativo, importo o dato personale reale è stato copiato. La fixture e il mapper coprono entrambe le forme e mantengono l'importo cliente `AMBIGUOUS`.
...
- Readback eBay storico: mapping fiscale e rimborsi verificati in sola lettura con il precedente keyset `botCF`. Dopo il consenso raccolto da SyncBay e l’installazione del token condiviso, `botCF 2` deve ripetere il readback e la prova firmata del relay prima della riapertura del worker.
