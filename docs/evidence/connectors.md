...
- Il keyset di destinazione è `botCF 2`; `EBAY_ACCOUNT_REFERENCE` identifica invece l'account venditore che la Identity API deve confermare e conserva il valore Production già verificato durante la rotazione. SyncBay riceve le notifiche Marketplace Account Deletion del keyset e dispone del relay durevole verso l'endpoint HTTPS pubblico di Hub Fatture. Hub Fatture verifica nuovamente la firma e cancella soltanto i dati del compratore non fiscalizzati.
- Contract check: `npm test -- src/integrations/connectors.test.ts`.
...
- Readback eBay storico: mapping fiscale e rimborsi verificati in sola lettura con il precedente keyset `botCF`. Dopo il nuovo consenso OAuth, `botCF 2` deve ripetere il readback e la prova firmata del relay prima della riapertura del worker.
