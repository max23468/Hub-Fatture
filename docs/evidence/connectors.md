...
- Il keyset di destinazione è `botCF 2`; `EBAY_ACCOUNT_REFERENCE=botCF` identifica invece l'account venditore che la Identity API deve confermare. SyncBay riceve le notifiche Marketplace Account Deletion del keyset e dispone del relay durevole verso l'endpoint HTTPS pubblico di Hub Fatture. Hub Fatture verifica nuovamente la firma e cancella soltanto i dati del compratore non fiscalizzati.
- Contract check: `npm test -- src/integrations/connectors.test.ts`.
