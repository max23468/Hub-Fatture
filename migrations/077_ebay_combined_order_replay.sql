-- Rilegge una sola volta gli ordini eBay provvisori recenti. La sincronizzazione
-- mirata risolve l'ID definitivo restituito da eBay e lascia al dominio il
-- consolidamento degli ordini ancora privi di legami fiscali.
...
UPDATE connections
SET status = CASE
      WHEN status = 'ERROR' AND last_error_code = 'PROVIDER_RESPONSE_INVALID'
        THEN 'CONNECTED'
      ELSE status
    END,
    last_synced_at = NULL,
    ...
    last_error_message_sanitized = CASE
      WHEN status = 'ERROR' AND last_error_code = 'PROVIDER_RESPONSE_INVALID'
        THEN NULL
      ELSE last_error_message_sanitized
    END,
    updated_at = now()
WHERE provider = 'EBAY'
  AND EXISTS (
