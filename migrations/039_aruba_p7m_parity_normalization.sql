WITH affected_accounts AS (
  SELECT DISTINCT runs.environment, runs.account_reference
...
WHERE runs.authority_mode = 'SHADOW'
  AND runs.status IN ('RUNNING', 'INCOMPLETE', 'COMPLETED')
  AND EXISTS (
    SELECT 1 FROM affected_accounts AS affected
    WHERE affected.environment = runs.environment
      AND affected.account_reference = runs.account_reference
  );
