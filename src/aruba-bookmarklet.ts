...
const failureMessage=(code)=>code==="POPUP_BLOCKED"?"Consenti l’apertura della finestra e riprova.":code==="ARUBA_ORIGIN_MISMATCH"?"Apri il pannello Aruba e usa lì questo preferito.":code==="ARUBA_FILTER_ACTIVE"?"Azzera il filtro Dal nella pagina Aruba e riprova.":code==="DOM_UNRECOGNIZED"?"La pagina Aruba non ha completato il caricamento previsto. Ricaricala e riprova.":code==="HUB_TIMEOUT"||code==="HUB_BRIDGE_TIMEOUT"?"Il collegamento con Hub Fatture è scaduto. Torna a Hub Fatture e riprova.":"La lettura si è interrotta prima del completamento. Torna a Hub Fatture e riprova.";
...
const assertDateFilterEmpty=()=>{const candidates=[...document.querySelectorAll('[data-aruba-filter-from],input[name="dataDa"]')].filter(visible);if(candidates.length!==1||!(candidates[0] instanceof HTMLInputElement))fail("DOM_UNRECOGNIZED");if(normalized(candidates[0].value))fail("ARUBA_FILTER_ACTIVE")};
const selectStream=async(stream,overlapFrom)=>{
  ...
  if(synthetic){synthetic.click();await sleep(100);if(overlapFrom)await applyDateFilter(overlapFrom);else assertDateFilterEmpty();return true}
  ...
  if(overlapFrom)await applyDateFilter(overlapFrom);else assertDateFilterEmpty();
  productionNext();
  return true;
};
