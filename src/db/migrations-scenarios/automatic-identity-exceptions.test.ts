import {
  ...
  EBAY_STRUCTURED_CUSTOMER_NAME_REPLAY,
...
test("l'upgrade rilegge il nome eBay già mostrato ma ancora non strutturato", async () => {
  const database = await temporaryDatabase("ebay_structured_customer_name_replay");
  const beforeReplay = await mkdtemp(
    path.join(os.tmpdir(), "hub-fatture-before-ebay-structured-name-replay-"),
  );
  try {
    await cp("migrations", beforeReplay, { recursive: true });
    await removeMigrationsFrom(beforeReplay, EBAY_STRUCTURED_CUSTOMER_NAME_REPLAY);
      ...
      await client.query(
        `INSERT INTO fiscal_profiles (version, status, profile_json)
         ...
         INSERT INTO connections
           (provider, environment, account_reference, encrypted_credentials, status,
            last_synced_at)
         VALUES ('EBAY', 'PRODUCTION', 'seller-structured-name', 'encrypted', 'CONNECTED',
                 '2026-09-19T10:00:00Z');
         INSERT INTO sync_cursors (provider, stream, cursor, overlap_from)
         VALUES ('EBAY', 'history_import', 'complete', '2026-01-01T00:00:00Z'),
                ('EBAY', 'orders', 'recent', '2026-09-19T09:00:00Z')`,
      );
      const customerId = (
        await client.query(
          `INSERT INTO customers
             (kind, match_key, display_name, billing_address_json, source_confidence,
              review_required)
           VALUES ('PRIVATE_IT', 'structured-name-replay', 'Nome Cognome', '{}', 'TAX_ID', true)
           RETURNING id`,
        )
      ).rows[0].id;
      const caseId = (
        await client.query(
          `INSERT INTO billing_cases
           ...
           VALUES ($1, '2026-07-27', 'EUR', 'NEEDS_REVIEW',
             '{"displayName":"Nome Cognome","reviewRequired":true}', 1)
           RETURNING id`,
          [customerId],
        )
      ).rows[0].id;
      await client.query(
        `INSERT INTO orders
           (provider, external_account_id, external_order_id, display_number,
            ...
            raw_snapshot_json, normalized_snapshot_json)
         VALUES ('EBAY', 'seller-structured-name', 'structured-name-order', 'E-NAME',
                 '2026-07-27T09:00:00Z', '2026-08-05T10:00:00Z', '2026-07-27',
                 'EUR', 1000, 'PAID', 'FULFILLED', 'GROUPED', $1, $2,
                 '{"customer":{"kind":"PRIVATE_IT","displayName":"Nome Cognome",
                    "taxIdentifiers":[{"type":"CODICE_FISCALE","countryCode":"IT",
                    "value":"RSSMRA80A01H501U"}]}}',
                 '{"customerReviewRequired":true,"customerSnapshot":{
                    "displayName":"Nome Cognome","reviewRequired":true}}')`,
        [customerId, caseId],
      );
    });

    const applied = await runMigrations({ connectionString: database.connectionString });
    assert.ok(applied.includes(EBAY_STRUCTURED_CUSTOMER_NAME_REPLAY));
    await withClient(database.connectionString, async (client) => {
      assert.deepEqual(
        (
          await client.query(
        ...
        { cursor: null, overlap_from: "2026-08-05 09:55:00+00" },
      );
      assert.equal(
