...
test(
  ...
  async () => {
    ...
    try {
      ...
      const canaryReadback = await import("./aruba-canary-readback.server.ts");
      const canaryReadiness = await import("./aruba-canary-readiness.server.ts");
      ...
      await database.getPool().query(
        `INSERT INTO aruba_sync_runs
          (id, environment, api_environment, account_reference, kind, authority_mode, status,
           window_start, window_end, checkpoint_start, checkpoint_end, lease_expires_at,
           completed_at, full_scan_completed_at)
         VALUES ('00000000-0000-4000-8000-000000000313', 'PRODUCTION', 'PRODUCTION',
           'qualified-production-account', 'FULL', 'CANONICAL', 'COMPLETED',
           now() - interval '1 hour', now(), now() - interval '1 hour', now(), now(), now(), now())`,
      );
      await database
        .getPool()
        .query(
          "INSERT INTO users (username, password_hash, can_approve) VALUES ('Codex', 'synthetic', true)",
        );
      const canaryBatchId = await database.withTransaction((client) =>
        arubaOutbound.createArubaApiBatch(
          client,
          [mixedDocuments[1]!],
          owner,
      ...
      await arubaOutbound.authorizeArubaApiDryRunQualification(canaryBatchId, owner, true);
      const canarySubmission = (
        await database
          .getPool()
          .query<{ id: string; xml_sha256: string }>(
            "SELECT id, xml_sha256 FROM aruba_submissions WHERE batch_id = $1",
            [canaryBatchId],
          )
      ).rows[0]!;
      await database.getPool().query(
        `DELETE FROM jobs WHERE type = 'aruba_dry_run_submission'
           AND payload_json ->> 'submissionId' = $1`,
        [canarySubmission.id],
      );
      await database.getPool().query(
        `UPDATE aruba_dry_run_qualifications
           SET status = 'SUCCEEDED', consumed_at = now(), completed_at = now()
           WHERE batch_id = $1`,
        [canaryBatchId],
      );
      await database
        .getPool()
        .query("UPDATE aruba_batches SET status = 'DRY_RUN_VALIDATED' WHERE id = $1", [
          canaryBatchId,
        ]);
      await database
        .getPool()
        .query("UPDATE aruba_submissions SET status = 'DRY_RUN_VALIDATED' WHERE id = $1", [
          canarySubmission.id,
        ]);
      await database.getPool().query(
        `INSERT INTO aruba_submission_attempts
           (id, submission_id, operation, attempt_number, request_fingerprint,
            xml_sha256, status, started_at, completed_at)
         VALUES ('00000000-0000-4000-8000-000000000314', $1, 'DRY_RUN', 1,
      ...
      await assert.rejects(
        arubaOutbound.authorizeArubaApiCanary(
          canaryBatchId,
          { id: 2, canApprove: true, requestId: "codex-canary-forbidden" },
          true,
        ),
        (error) => error instanceof AppError && error.code === "ARUBA_OPERATION_FORBIDDEN",
      );
      const canary = await arubaOutbound.authorizeArubaApiCanary(canaryBatchId, owner, true);
      assert.match(canary.permitId, /^[0-9a-f-]{36}$/);
      assert.equal(canary.queued, 1);
      assert.deepEqual(
        (
          await database.getPool().query(
            `SELECT permits.environment, permits.consumed_at IS NOT NULL AS consumed,
                    permits.expired_at IS NOT NULL AS expired,
                    submissions.status AS submission_status, jobs.max_attempts
             FROM aruba_canary_permits AS permits
             JOIN aruba_submissions AS submissions ON submissions.batch_id = permits.batch_id
             JOIN jobs ON jobs.type = 'aruba_send_submission'
               AND jobs.payload_json ->> 'submissionId' = submissions.id::text
             WHERE permits.batch_id = $1`,
            [canaryBatchId],
          )
        ).rows[0],
        {
          ...
          consumed: false,
          expired: false,
          submission_status: "READY_TO_SEND",
          max_attempts: 1,
        },
      );
      await database.getPool().query(
        `UPDATE jobs SET run_at = now() + interval '1 hour'
         WHERE status = 'PENDING' AND NOT (
           type = 'aruba_send_submission' AND payload_json ->> 'submissionId' = $1
         )`,
        [canarySubmission.id],
      );
      await database
        .getPool()
        .query("UPDATE aruba_canary_permits SET consumed_at = now() WHERE batch_id = $1", [
          canaryBatchId,
        ]);
      await database.getPool().query(
        `INSERT INTO aruba_submission_attempts
           (id, submission_id, operation, attempt_number, request_fingerprint,
            xml_sha256, status, started_at)
         VALUES ('00000000-0000-4000-8000-000000000315', $1, 'SEND', 1,
           repeat('b', 64), $2, 'RUNNING', now())`,
        [canarySubmission.id, canarySubmission.xml_sha256],
      );
      const canaryJob = await jobs.claimJob("aruba-canary-recovery-worker");
      assert.equal(canaryJob?.type, "aruba_send_submission");
      const canaryRecovery = await arubaOutbound.runArubaApiOutboundJob(canaryJob!);
      assert.deepEqual(canaryRecovery, {
        accepted: false,
        unknownRemoteState: true,
        submissionId: canarySubmission.id,
        batchId: canaryBatchId,
      });
      assert.equal(await jobs.completeJob(canaryJob!, canaryRecovery), true);
      assert.deepEqual(
        (
          await database.getPool().query(
            `SELECT batches.status AS batch_status, batches.requires_reconciliation,
                    submissions.status AS submission_status, attempts.status AS attempt_status
             FROM aruba_batches AS batches
             JOIN aruba_submissions AS submissions ON submissions.batch_id = batches.id
             JOIN aruba_submission_attempts AS attempts ON attempts.submission_id = submissions.id
             WHERE batches.id = $1 AND attempts.operation = 'SEND'`,
            [canaryBatchId],
          )
        ).rows[0],
        {
        ...
        await database.getPool().query<{ id: string }>(
          `INSERT INTO aruba_remote_documents
            (environment, account_reference, remote_id, document_type, fiscal_year, series,
             ...
             remote_status_observed_at, xml_sha256, metadata_digest, origin)
           SELECT 'PRODUCTION', 'qualified-production-account', 'canary-remoto-sintetico',
             ...
             'DELIVERED', now(), documents.xml_sha256, repeat('c', 64), 'UNKNOWN'
      ...
      await database.getPool().query(
        `INSERT INTO aruba_files
          (document_id, submission_id, remote_document_id, storage_object_id, kind, metadata_json)
         SELECT NULL, NULL, $1, documents.storage_object_id, 'ARUBA_XML', '{}'::jsonb
         FROM documents WHERE documents.id = $2`,
        [canaryRemote.id, mixedDocuments[1]!.id],
      );
      await database.getPool().query(
        `INSERT INTO aruba_submission_attempts
           (id, submission_id, operation, attempt_number, request_fingerprint,
            xml_sha256, status)
         VALUES ('00000000-0000-4000-8000-000000000316', $1, 'READBACK', 1,
           repeat('d', 64), $2, 'PENDING')`,
        [canarySubmission.id, canarySubmission.xml_sha256],
      );
      assert.deepEqual(
        await database.withTransaction((client) =>
          canaryReadback.reconcileCanarySubmissionReadback(client, canaryRemote.id),
        ),
        {
          submissionId: canarySubmission.id,
          batchId: canaryBatchId,
      ...
      assert.deepEqual(
        (
          await database.getPool().query(
            `SELECT batches.status AS batch_status, batches.requires_reconciliation,
                    submissions.status AS submission_status, submissions.remote_id,
                    attempts.status AS readback_status, remote.origin
             FROM aruba_batches AS batches
             JOIN aruba_submissions AS submissions ON submissions.batch_id = batches.id
             JOIN aruba_submission_attempts AS attempts ON attempts.submission_id = submissions.id
               AND attempts.operation = 'READBACK'
             JOIN aruba_remote_documents AS remote ON remote.id = $2
             WHERE batches.id = $1`,
            [canaryBatchId, canaryRemote.id],
          )
        ).rows[0],
        {
          batch_status: "RECONCILED",
          requires_reconciliation: false,
          submission_status: "DELIVERED",
          remote_id: "canary-remoto-sintetico",
          readback_status: "SUCCEEDED",
          origin: "HUB_SUBMISSION",
        },
      );
      await database
        .getPool()
